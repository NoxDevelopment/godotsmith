...
Functions can accept variable numbers of arguments using rest parameter syntax:
...
# Rules:
# - Only one rest parameter, must be last
# - Rest parameter type is always Array (not Array[Type])
# - Cannot unpack/spread on call sites (use callv() instead)
```

### Abstract Classes and Methods (4.5)

Prevent instantiation and require subclass implementation:
...
class Shape extends Node2D:
	...
	func describe() -> String:
		return "Area: %.2f" % get_area()  # Can call abstract methods
...
# Shape.new()  # ERROR: cannot instantiate abstract class
# Attaching Shape.gd to a node also fails
```
...
# Deep copy (recursive, fully independent):
var deep = original.duplicate_deep()  # Arrays, Dictionaries, Resources
```
...
# Compile-time type checking on keys and values
```

## Godot 4.6 Features

### Inverse Kinematics Framework (4.6)

New IK modifier nodes for 3D character animation:

```gdscript
# Available IK solvers (add as children of Skeleton3D):
# - IKModifier3D — base class
# - TwoBoneIK3D — arms/legs (elbow/knee targeting)
# - FABRIK3D — chain solving (tentacles, tails, spines)
# - CCDIK3D — cyclic coordinate descent
# - JacobianIK3D — full Jacobian solver

# Example: arm reaching for object
var ik: TwoBoneIK3D = $Skeleton3D/TwoBoneIK
ik.target_node = $ReachTarget.get_path()
```

### AgX Tonemapper Parameters (4.6)

```gdscript
# New environment tonemapping controls:
var env := Environment.new()
env.tonemap_mode = Environment.TONE_MAPPER_AGX
# New params: agx_white, agx_contrast for HDR tuning
```

### Screen Space Reflection Overhaul (4.6)

Half-resolution and full-resolution SSR modes available. Set in Environment or project settings.

### D3D12 Default on Windows (4.6)

New Windows projects default to Direct3D 12. For explicit Vulkan:
```ini
[rendering]
...
### LibGodot Embedding (4.6)

Godot can now be embedded directly into custom applications on Windows/Linux/macOS via the LibGodot API.
